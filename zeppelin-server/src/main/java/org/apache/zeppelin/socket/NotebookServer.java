...
import org.apache.zeppelin.utils.PermissionJdbcUtil;
...
import static org.apache.zeppelin.server.ZeppelinServer.notebookWsServer;
...
public class NotebookServer extends WebSocketServlet
  ...
  private void runParagraph(NotebookSocket conn, HashSet<String> userAndRoles, Notebook notebook,
    ...
    //Add database permission controller
    String[] roles = fromMessage.roles.substring(2, fromMessage.roles.length() - 2).split(",");
    String textInLowerCase = ((String) fromMessage.get("paragraph")).toLowerCase();
    for (String role : roles) {
      String dbFilter[] = new String[1000];
      String errorDb = "";
      try {
        ResultSet resultSet = PermissionJdbcUtil.executeQuery("select * from zeppelin_permission_db where role='" + role + "'");
        if (!resultSet.next()) {
          LOG.error("权限查询为空！");
          conn.send(serializeMessage(new Message(OP.AUTH_INFO).put("info",
                  "权限查询为空，请联系管理员")));
        ...
        String[] allowDb = resultSet.getString("allow_db").split(",");
        String[] notAllowedDb = resultSet.getString("not_allowed_db").split(",");
        resultSet.close();
        int dbFilterIndex = 0;
        if (allowDb[0] != null) {
          for (int i = 0; i < notAllowedDb.length; i++) {
            boolean isAllow = false;
            for (int j = 0; j < allowDb.length; j++) {
              if (notAllowedDb[i].equals(allowDb[j])) {
                isAllow = true;
                break;
              }
            }
            if (!isAllow) {
              dbFilter[dbFilterIndex++] = notAllowedDb[i];
            }
          }
        }
      } catch (Exception e) {
      ...
      if (fromMessage.roles.contains(role)) {
        boolean permission = true;
        for (String tmp : dbFilter) {
          if (tmp == null) {
            break;
          }
          if (textInLowerCase.contains(tmp)) {
            permission = false;
            errorDb = tmp;
            break;
          }
        ...
        if (!permission) {
          LOG.info("Insufficient privileges to database:{}.db", errorDb);
          conn.send(serializeMessage(new Message(OP.AUTH_INFO).put("info",
                  "Insufficient privileges to database:" + errorDb + ".db\n\n" + "Allowed users or roles: admin&develop" + "\n\n" + "But the user " + fromMessage.principal + " belongs to: " + userAndRoles
                          .toString())));
          return;
        }
      }
...
//        LOG.info("自动重新建立连接！");
//        InterpreterRestApi.interpreterSettingManager.close("spark221");
//      }
      if (p != null) {
...
//          if (job.getReturn().toString().contains("SocketException")
//                  || job.getErrorMessage().contains("SocketException")) {
//            LOG.warn("连接断开了，即将lazy restart");
//            InterpreterRestApi.interpreterSettingManager.close("spark221");
//          }

//          //关闭interpreter spark221
//          try {
//            ZeppelinConfiguration conf = ZeppelinConfiguration.create();
//            InterpreterSettingManager interpreterSettingManager = new InterpreterSettingManager(conf, notebookWsServer,
//                    notebookWsServer, notebookWsServer);
////            interpreterSettingManager.restart("spark221");
//
//
//            InterpreterRestApi interpreterRestApi = new InterpreterRestApi(interpreterSettingManager, notebookWsServer);
//            interpreterRestApi.restartSetting("","spark221");
//            interpreterSettingManager.restart("spark221", "2DJETUEVQ", SecurityUtils.getPrincipal());
//          }catch (Exception e){
//            LOG.warn("关闭连接失败");
//            e.printStackTrace();
//          }
