...
function ParagraphCtrl($scope, $rootScope, $route, $window, $routeParams, $location,
  ...
  const filter = ['create table ', 'create external table ', 'create temporary table ',
    'insert overwrite ', 'load data ', 'truncate ', 'drop table ', 'drop database ', 'drop partition '];
    ...
    runParagraph: function(paragraphId) {
      if (paragraphId) {
        ...
        if (filtered.length === 1) {
          ...
          const lowerCaseSql = paragraph.text.toLowerCase();
          let sql = lowerCaseSql[0];
          for (let j = 1; j < lowerCaseSql.length; j++) {
            if (lowerCaseSql[j] === '↵' || lowerCaseSql[j] === '\n'
              || lowerCaseSql[j] === '\t' || lowerCaseSql[j] === ' ') {
              if (lowerCaseSql[j - 1] === '↵' || lowerCaseSql[j - 1] === '\n'
                || lowerCaseSql[j - 1] === '\t' || lowerCaseSql[j - 1] === ' ') {
                continue;
              } else {
                sql += ' ';
              }
  ...
  $scope.runParagraph = function(paragraphText, digestRequired, propagated) {
    ...
    const lowerCaseSql = paragraphText.toLowerCase();

    if(lowerCaseSql.trim()==='restart') {
    ...
    setTimeout($scope.autoRestart(), 3000);
  };

  let flag = 0;
  $scope.autoRestart = function() {
    if (($scope.paragraph.results.msg[0].data.indexOf('TThreadPoolServer') !== -1
        || $scope.paragraph.results.msg[0].data.indexOf('TTransportException') !== -1
        || $scope.paragraph.results.msg[0].data.indexOf('HiveSQLException') !== -1)
      && flag === 0) {
      flag = 1;
      $http.put(baseUrlSrv.getRestApiBase() + '/interpreter/setting/restart/spark221')
        .success(function(data, status, headers, config) {
