...
function ResultCtrl($scope, $rootScope, $route, $window, $routeParams, $location,
  ...
  $scope.exportFreely = function(paragraphText) {
    const sql = $scope.getEditorValue();
    if ($scope.canExport === 1) {
      $scope.paragraph.status = 'EXPORTING';
      console.log('exporting set, sql got:' + sql);
      console.log('paragraphText :' + paragraphText);
      $http({
        method: 'post',
        url: 'http://argus-api.pre.2dfire.net/rule/exportZeppelin',
        headers: {'X-token': 'Zeppelin'},
        params: {
          paragraphText: paragraphText,
          zeppelinRoles: $rootScope.ticket.roles,
        },
      }).success(function(data) {
        if (data.data === '-1') {
          alert('请进行select查询 or 权限不足，请发邮件申请');
        } else {
          $scope.canExport = 0;
          $scope.exportId = data.data;
          alert('取数请求提交成功，请稍后点击获取按钮~');
        }
      }).error(function(err) {
        alert('当前取数请求过多，请稍后再试~');
      });
    } else {
      alert('请勿重复请求导出~');
    }
  ...
  $scope.getYourData = function() {
    if ($scope.canExport === 0) {
      ...
      if ($scope.exportId === '-1') {
        ...
        $http({
          method: 'get',
          url: 'http://argus-api.pre.2dfire.net/rule/getExportResult',
          headers: {'X-token': 'Zeppelin'},
          params: {
            taskId: taskId,
          },
          ...
          $scope.paragraph.status = 'SUCCESS';
          if (data.data && data.data.downloadUrl) {
            $scope.canExport = 1;
            $scope.exportId = '-1';
            alert('查询成功！下载地址：' + data.data.downloadUrl);
          } else {
            alert('计算中，请耐心等待后，再重试~');
          }
        }).error(function(err) {
