...
function NotebookCtrl($scope, $route, $routeParams, $location, $rootScope,
  ...
  $scope.runAllParagraphs = function(noteId) {
    for (let i = 0; i < $scope.note.paragraphs.length; i++) {
      if ($scope.note.paragraphs[i] === null
        || $scope.note.paragraphs[i] === undefined) {
        continue;
      }
      if ($scope.note.paragraphs[i].text === null
        || $scope.note.paragraphs[i].text === ''
        || $scope.note.paragraphs[i].text === undefined) {
        continue;
      }
      const lowerCaseSql = $scope.note.paragraphs[i].text.toLowerCase();
        ...
        buttons: [
          {
            ...
            action: function(dialog) {
              ...
              $scope.permissions.writers = [$rootScope.ticket.principal];
              $scope.permissions.runners = [$rootScope.ticket.principal];
